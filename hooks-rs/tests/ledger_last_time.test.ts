...
const HOOK_NAME = "ledger_last_time";

describe("ledger_last_time.rs", () => {
  let client: Client;
  let alice: Wallet;
  let bob: Wallet;
  ...
  beforeAll(async () => {
    ...
    // rw3nfJSVBWBbJiVYRzq7ExFgU1VgD8tnq2
    alice = Wallet.fromSecret(`snbePNZWBkkYFiaj3YGaeuK4TeGu6`);
    // rha3hBEaA8LnTXNPz2UP6GT4qv4DfBCPSj
    bob = Wallet.fromSecret(`snZd1z4ikJtPsrC7TYyse9vaVZe7r`);
  ...
  it(
    ...
    async () => {
      ...
      // so this time looks a bit different from Unix Epoch time
      const { close_time } = ledgerResponse.result.ledger;
      ...
      const ledgerCloseTimeFromHook = TestUtils.deserializeHexStringAsBigInt(
        hookExecutions.executions[0].HookReturnCode.toString(),
      );
      expect(hookExecutions.executions[0].HookReturnString).toMatch("");
      expect(ledgerCloseTimeFromHook).toBeGreaterThanOrEqual(close_time);
    },
    3 * 60_000,
  );
});
